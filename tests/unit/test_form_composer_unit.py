...
def test_compose_form_MT_emits_three_tabs(sample_entity, sample_endpoints):
    """Growth-41: MT pattern renders 3-tab shell with lazy-load dispatcher."""
    out = compose_form(sample_entity, sample_endpoints, pattern="MT")
    assert '<Tab id="tabMain"' in out
    assert '<Tabpage id="tab_basic"' in out
    assert '<Tabpage id="tab_related"' in out
    assert '<Tabpage id="tab_history"' in out
    assert "fn_tab_changed" in out
    assert "fn_load_current_tab" in out
    assert "getRowCount()" in out


def test_compose_form_TG_left_tree_right_grid(sample_entity, sample_endpoints):
    """Growth-41: TG pattern renders category tree on left + entity grid on right."""
    out = compose_form(sample_entity, sample_endpoints, pattern="TG")
    assert 'binddataset="dsCategory"' in out
    assert 'treeitemcontrol' in out
    assert "fn_category_selected" in out
    assert "grd_detail" in out
    assert "category_id" in out      # default FK fallback


def test_compose_form_PS_is_popup(sample_entity, sample_endpoints):
    """Growth-41: PS pattern emits popup-classed form with double-click select."""
    out = compose_form(sample_entity, sample_endpoints, pattern="PS")
    assert 'classname="Popup"' in out
    assert "fn_select_row" in out
    assert "fnReceivePopupData" in out
    assert "oncelldblclick" in out
    assert "btn_cancel" in out
    # PS is read-only-lookup — must not emit save transaction
    assert 'dataList=ds' not in out


def test_compose_form_MDS_header_lines(sample_entity, sample_endpoints):
    """Growth-41: MDS pattern emits header+lines with single combined save."""
    child_entity = _mk_child_entity()
    child_endpoints = _mk_child_endpoints()
    out = compose_form(
        sample_entity, sample_endpoints, pattern="MDS",
        child_entity=child_entity, child_endpoints=child_endpoints,
        child_fk_column="sales_order_id",
    )
    assert "grd_master" in out
    assert "grd_lines" in out
    # combined save transaction
    assert "dataList=ds" in out and ",dsChildList=ds" in out
    # header BindItem fields generated by header_field.xml.j2
    assert "edt_h_" in out or "cal_h_" in out
    assert "<Bind id=\"bnd_h_" in out
    assert "btn_new" in out and "btn_save" in out
